...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        r, g, b = pixel
        transformed_pixel = [
            sum([r * matrix[0][i], g * matrix[1][i], b * matrix[2][i]]) for i in range(3)
        ]
        filtered_pixels.append(tuple(int(min(255, max(0, x))) for x in transformed_pixel))
...
def extract_end_bits(num_end_bits, pixel):
    ...
    def extract_bits(value, num_bits):
        return int(bin(value)[-num_bits:], 2)

    if isinstance(pixel, tuple):  # For RGB pixel
        return tuple(extract_bits(c, num_end_bits) for c in pixel)
    else:  # For BW pixel
        return extract_bits(pixel, num_end_bits)
...
def reveal_bw_image(filename):
    ...
    revealed_pixels = [extract_end_bits(1, p) for p in pixels]
    size = Image.open(filename).size
    return pix_to_img(revealed_pixels, size, 'L')
...
def reveal_color_image(filename):
    ...
    """
    pixels = img_to_pix(filename)
    revealed_pixels = [extract_end_bits(3, p) for p in pixels]
    size = Image.open(filename).size
